...
async function adminBroadcastStatus(req){
  ...
  try{
    ...
    const nextRows=await sql`select id,broadcast_id,ordinal,kind,source_id,source_room,title,artist,state,selection_reason,scheduled_for from broadcast_items where broadcast_id=${broadcast.id}::uuid and state in ('planned','ready') order by sort_key,ordinal limit 1`;
...
async function adminBroadcastLineup(req,url){
  ...
  try{
    ...
    const items=await sql`select id,broadcast_id,ordinal,kind,source_id,source_room,title,artist,state,selection_reason,scheduled_for,started_at,ended_at,failure_reason,created_at,updated_at from broadcast_items where broadcast_id=${broadcast.id}::uuid order by sort_key,ordinal limit 250`;
...
async function adminBroadcastReorder(req){
  const gate=await requireAdmin(req);if(gate.response)return gate.response;
  const commandId=broadcastCommandId(req);if(!commandId)return json({error:'Valid Idempotency-Key required'},400);
  let existing;try{existing=await existingBroadcastCommand(sql,commandId)}catch(error){if(broadcastSchemaMissing(error))return json({error:'Broadcast schema is not installed'},503);throw error}
  ...
  try{
    const current=await sql`select id::text as id from broadcast_items where broadcast_id=${broadcastId}::uuid and state in ('planned','ready') order by sort_key,ordinal`;
