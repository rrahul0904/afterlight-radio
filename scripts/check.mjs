...
for(const needle of ['Neon-Connection-String','Neon-Raw-Text-Output','DATABASE_URL','STRIPE_WEBHOOK_SECRET','/api/ready','/api/support','/api/stripe/webhook','/api/library/provider/status','/api/library/provider/search','/api/library/provider/lyrics','/api/library/provider/stream','/api/library/provider/artwork','getLyricsBySongId','normalizedProviderLyrics',"lyricsUrl:'/api/library/provider/lyrics?id='",'/api/admin/summary','/api/admin/users','AFTERLIGHT_ADMIN_EMAILS','neon_auth."user"','Admin access required','NAVIDROME_BASE_URL','NAVIDROME_USERNAME','NAVIDROME_TOKEN','NAVIDROME_SALT','External music library must use HTTPS','External music library host is not allowed','backend:\'neon-function\'','buy.stripe.com'])if(!neonFn.includes(needle))throw new Error('Production Neon Function missing: '+needle);
