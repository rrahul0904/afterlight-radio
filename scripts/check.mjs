...
for(const needle of ["/api/auth/","/get-session","/api/health","/api/ready","/api/config","/api/me","/api/preferences","/api/checkout","/api/portal","/api/events","/api/stripe/webhook","Stripe-Signature","2026-07-29.dahlia","buy.stripe.com","locked_prefilled_email"]){
...
for(const needle of ["handleApi","bodyParser:false","getSetCookie","runtime-secrets.json","process.env"])if(!vercel.includes(needle))throw new Error('Vercel adapter missing: '+needle);
