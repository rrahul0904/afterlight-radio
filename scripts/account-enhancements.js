(()=>{
  ...
  if(button){
    ...
    render=function(){originalRender();const hasBilling=!!me?.subscription?.stripe_customer_id;if(hasBilling&&!config?.portalEnabled){button.disabled=false;button.textContent='Billing support';if(note)note.textContent='Billing changes and subscription cancellations are handled through Afterlight Support.'}else if(config?.portalEnabled){button.textContent='Manage billing'}};
