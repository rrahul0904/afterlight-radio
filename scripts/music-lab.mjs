...
select,textarea{font:inherit;color:var(--ink);background:#fffaf3;border:1px solid var(--line);padding:9px}.decision{margin-top:12px;max-width:220px}.notes{margin-top:12px}textarea{min-height:90px;resize:vertical}
...
for(const room of rooms){
  ...
  for(let i=0;i<count;i++){
    ...
    await writeFile(path.join(outRoot,'candidates',file),rendered.bytes);
  ...
  summary.push({room:room.slug,candidates:candidates.length,minDuration:Math.min(...candidates.map(c=>c.metrics.durationSeconds)),maxDuration:Math.max(...candidates.map(c=>c.metrics.durationSeconds)),technicalPass:candidates.filter(c=>c.metrics.technicalPass).length,out:outRoot});
}
