...
for(const needle of ['create table if not exists public.broadcasts','create table if not exists public.broadcast_items','create table if not exists public.broadcast_events',"unique (broadcast_id, ordinal)","actor text not null default 'director'"]){
...
for(const source of [core,neonFn]){
  for(const needle of ['/api/admin/broadcast/status','/api/admin/broadcast/lineup','Admin access required','broadcast_items','broadcasts']){
    if(!source.includes(needle))throw new Error('Broadcast API parity missing: '+needle);
  }
  for(const forbidden of ["p==='/api/admin/broadcast/start'","pathname==='/api/admin/broadcast/start'","p==='/api/admin/broadcast/stop'","pathname==='/api/admin/broadcast/stop'"]){
    if(source.includes(forbidden))throw new Error('Phase 1 must remain read-only: '+forbidden);
  }
}

console.log('PASS broadcast-phase1: deterministic 36-track planner, director-only state transitions, Neon schema, and protected read-only admin status/lineup API parity');
